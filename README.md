...
1. **Map/reduce histograms:** `run_event_selection_batched.sh` surveys input `.df` files into
   ≤1 GiB jobs (`event_selection_batch_survey.py`), runs the selection pipeline per job
   (`event_selection_batch_map.py`), then `event_selection_aggregate.py` merges and plots.
