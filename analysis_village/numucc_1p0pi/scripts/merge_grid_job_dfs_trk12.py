...
Same as ``merge_grid_job_dfs.py`` but uses ``trk1`` / ``trk2`` HDF columns instead
of ``mu`` / ``p`` for FV cuts and TKI inputs.
...
CATHODE_INSET = 10
def perTPC_cut(df):
    in_TPC1_cut = InFV(df.slc.vertex, det="SBND_TPC1", incathode=CATHODE_INSET) & InFV(df.trk1.pfp.trk.end, det="SBND_TPC1", incathode=CATHODE_INSET) & InFV(df.trk2.pfp.trk.end, det="SBND_TPC1", incathode=CATHODE_INSET)
    in_TPC2_cut = InFV(df.slc.vertex, det="SBND_TPC2", incathode=CATHODE_INSET) & InFV(df.trk1.pfp.trk.end, det="SBND_TPC2", incathode=CATHODE_INSET) & InFV(df.trk2.pfp.trk.end, det="SBND_TPC2", incathode=CATHODE_INSET)
    perTPC_cut = in_TPC1_cut | in_TPC2_cut
    return perTPC_cut

def contTPC1_cut(df):
    cont_TPC1_cut = InFV(df.slc.vertex, det="SBND_TPC1", incathode=CATHODE_INSET) & InFV(df.trk1.pfp.trk.end, det="SBND_TPC1", incathode=CATHODE_INSET) & InFV(df.trk2.pfp.trk.end, det="SBND_TPC1", incathode=CATHODE_INSET)
    return cont_TPC1_cut

def contTPC2_cut(df):
    cont_TPC2_cut = InFV(df.slc.vertex, det="SBND_TPC2", incathode=CATHODE_INSET) & InFV(df.trk1.pfp.trk.end, det="SBND_TPC2", incathode=CATHODE_INSET) & InFV(df.trk2.pfp.trk.end, det="SBND_TPC2", incathode=CATHODE_INSET)
...
def add_notebook_derived_columns(merged: dict[str, pd.DataFrame], fv_cut: str) -> None:
    ...
    if "mcnu" in merged and len(merged["mcnu"]) > 0:
        ...
        try:
            mc_trk1df = mc_nu_df["mc"]["trk1"]
            mc_trk2df = mc_nu_df["mc"]["trk2"]
            mc_P_trk1_col = pad_column_name(("totp",), mc_trk1df)
            mc_P_trk2_col = pad_column_name(("totp",), mc_trk2df)
            tki_mc = get_cc1p0pi_tki(mc_trk1df, mc_trk2df, mc_P_trk1_col, mc_P_trk2_col)
    ...
    if "evt" in merged and len(merged["evt"]) > 0:
        ...
        try:
            slc_trk1df = mc_evt_df["trk1"]["pfp"]["trk"]["truth"]["p"]
            slc_trk2df = mc_evt_df["trk2"]["pfp"]["trk"]["truth"]["p"]
            slc_P_trk1_col = pad_column_name(("totp",), slc_trk1df)
            slc_P_trk2_col = pad_column_name(("totp",), slc_trk2df)
            tki_reco = get_cc1p0pi_tki(slc_trk1df, slc_trk2df, slc_P_trk1_col, slc_P_trk2_col)
            for var_name in _TKI_VAR_NAMES:
