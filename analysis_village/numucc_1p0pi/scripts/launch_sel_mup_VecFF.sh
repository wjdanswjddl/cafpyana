...
#               GENIE_VAR_SAVE_NAMES (default: all final-stage vars)
#
set -euo pipefail
...
# Empty = no allowlist = every final-stage var, matching the 23 the Ar23p knobs got.
...
echo "===== LAUNCH $(date -Is) final VecFF vars=${GENIE_VAR_SAVE_NAMES:-<all final-stage>} workers=${WORKERS} =====" | tee -a "$LOG"
