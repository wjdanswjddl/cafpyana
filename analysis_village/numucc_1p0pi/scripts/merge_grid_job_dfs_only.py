#!/usr/bin/env python3
"""
Merge grid-job ``.df`` (HDF5) outputs into ``<df_dir>/merged/`` (concat only).

Same chunking, ``__ntuple`` remapping, and HDF write logic as
``merge_grid_job_dfs.py``, but **no** post-merge processing: no FV cuts, no
topo/genie categories, no TKI variables.
...
- **Plan chunks from disk sizes**: read each input's ``os.path.getsize`` up front,
  ...
  budget derived from ``--max-chunk-mb`` and ``--size-budget-fraction``.
- **Merge one planned chunk at a time**: load only that chunk's files, concat,
  write (with optional row-split if a chunk still exceeds ``--max-chunk-mb``),
  then drop the merged dict and ``gc.collect()``.
...
def flush_merged_to_outputs(
    ...
    *,
    ...
    """
    Split by ``max_bytes`` if needed, write HDF(s), append JSONL log lines.
    Returns next ``chunk_idx``.
    """
...
def main() -> int:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "group must stay <= ``max_chunk_mb * fraction`` (default 0.92)."
        ),
    )
    p.add_argument(
        "--output-stem",
    ...
    out_dir = path.join(df_dir, "merged")
